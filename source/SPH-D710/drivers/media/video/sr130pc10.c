...
CAM_ERROR_MSG(&client->dev, "%s: sr130pc10_i2c_set_config_register E: %s, %d\n", __func__, name, err);
...
CAM_ERROR_MSG(&client->dev, "%s: sr130pc10_i2c_set_config_register X: %s, %d\n", __func__, name, err);
...
static void sr130pc10_init_parameters(struct v4l2_subdev *sd)
{
    ...
    state->capture_mode = V4L2_PIX_FMT_MODE_PREVIEW; //V4L2_MODE_PREVIEW;
...
static int sr130pc10_enum_framesizes(struct v4l2_subdev *sd, struct v4l2_frmsizeenum *fsize)
{
    ...
    if(state->capture_mode == V4L2_PIX_FMT_MODE_CAPTURE) //V4L2_MODE_CAPTURE)
    {
...
static int sr130pc10_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch(ctrl->id) 
	{
		...
		{
			state->check_dataline = 0; //ctrl->value;
			err = 0;
			break;	
        }
...
static int sr130pc10_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	CAM_ERROR_MSG(&client->dev, "1.3MP camera SR130PC10 loaded.\n");
